...
class DjangoMiddlewareTest(DjangoTraceTestCase):
    ...
    @modify_settings(
        MIDDLEWARE={
            'remove': 'ddtrace.contrib.django.TraceMiddleware',
            'prepend': [
                'ddtrace.contrib.django.TraceMiddleware',
                'tests.contrib.django.app.middlewares.ThrowErrorMiddleware',
            ],
        },
        MIDDLEWARE_CLASSES={
            'remove': 'ddtrace.contrib.django.TraceMiddleware',
    ...
    def test_middleware_throwing_error(self):
        # ensures we trace the error generated by a middleware
        url = reverse('fn-view')
        response = self.client.get(url)
        eq_(response.status_code, 500)
